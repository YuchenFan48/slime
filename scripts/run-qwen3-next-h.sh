...
source "${SCRIPT_DIR}/models/qwen3-next-2B-A0.5B-infllmv2.sh"
...
CKPT_ARGS=(
   ...
   --hf-checkpoint /mnt/shared-storage-user/p1-shared/yuchenzhang/qwen-3-next-2B-A0.5B-infllm
   --ref-load /mnt/shared-storage-user/p1-shared/yuchenzhang/qwen-3-next-2B-A0.5B-torch_dist-infllm-1125
   # Training from scratch - use save path as load path (will initialize randomly)
   --load /mnt/shared-storage-user/p1-shared/yuchenzhang/qwen-3-next-2B-infllmv2-16heads-mqa-hdim128-1125/
   --save /mnt/shared-storage-user/p1-shared/yuchenzhang/qwen-3-next-2B-infllmv2-16heads-mqa-hdim128-1125/
   --save-interval 4096
   --no-load-optim 
...
OPTIMIZER_ARGS=(
   ...
   # ⚠️ CRITICAL: Add gradient clipping to prevent numerical instability
   # Especially important for training from scratch with InfLLM V2
   --clip-grad 1.0     # Clip gradient norm to 1.0 (conservative for stability)
...
RUNTIME_ENV_JSON="{
  \"env_vars\": {
    \"PYTHONPATH\": \"/mnt/shared-storage-user/p1-shared/yuchenzhang/Megatron-LM/:/mnt/shared-storage-user/p1-shared/yuchenzhang/slime:/mnt/shared-storage-user/p1-shared/yuchenzhang/infllmv2_cuda_impl\",
...
if [ "$RANK" == "0" ]; then
    ...
    -- python3 /mnt/shared-storage-user/p1-shared/yuchenzhang/slime/train_async.py \
