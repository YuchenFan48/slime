...
# Add hyper-connections identifier to run name
HYPER_CONNECTIONS_STREAMS=${HYPER_CONNECTIONS_STREAMS:-4}  # Default to 4 streams
WANDB_RUN_NAME="${MODEL_NAME}-${TOTAL_PARAMS}-A${ACTIVE_PARAMS}-lr${LEARNING_RATE}-bsz${GLOBAL_BATCH_SIZE}-hyper-conn-${HYPER_CONNECTIONS_STREAMS}streams-kimi"
...
CKPT_ARGS=(
   ...
   --load /apdcephfs/mnt/cephfs/users/yuchenfan/qwen3-kda-new-data-4m-hyper-conn/
   --save /apdcephfs/mnt/cephfs/users/yuchenfan/qwen3-kda-new-data-4m-hyper-conn/
   --save-interval 2048
)
...
SFT_ARGS=(
   ...
   --prompt-data /apdcephfs/mnt/cephfs/users/yuchenfan/pretraining/ppl_data/pile/validation_sample_truncated.jsonl
   --input-key text
   --rollout-shuffle
   --num-rollout 1000000
...
WANDB_ARGS=(
   ...
   --wandb-group qwen-kimi-olmo3-hyper-conn
...
# Enable Hyper-Connections with configurable number of residual streams
HYPER_CONNECTIONS_ARGS=(
   --use-hyper-connections
   --num-residual-streams ${HYPER_CONNECTIONS_STREAMS:-4}
   --hyper-connections-dropout 0.0
   --num-fracs 1
)
...
if [ $MASTER_ADDR == $LOCAL_ADDR ]; then
    ...
    echo "Hyper-Connections enabled with ${HYPER_CONNECTIONS_STREAMS:-4} residual streams"
    ray stop --force
    if [ $NNODES -gt 1 ]; then
