"""
Multi-file async data source with checkpoint resume support.

This module provides an enhanced data source that:
1. Supports loading from multiple .jsonl/.parquet files in a directory
2. Uses ThreadPoolExecutor for parallel file loading
3. Supports checkpoint resume by filename matching
"""
...
class RolloutDataSourceMultiFile:
    """
    Multi-file async data source with background loading and checkpoint resume.
    
    Features:
    - Scans directory for .jsonl and .parquet files
    - Parallel file loading with ThreadPoolExecutor
    - Checkpoint save/load with filename-based resume
    - Detailed logging
    """
    ...
    def __init__(self, args):
        ...
        # Optional: filter out specific files (customize as needed)
        files_to_skip = set()  # Add filenames to skip if needed
        ...
        if len(self.data_files) < original_count:
            ...
            self._log(f"[Init] Manually skipped {skipped_count} files found in blocklist")
    ...
    def load(self, rollout_id=None):
        ...
        # 2. Find resume index by filename matching
        resume_index = -1
...
class RolloutDataSourceMultiFileWithBuffer(RolloutDataSourceMultiFile):
    """Multi-file data source with buffer support"""
